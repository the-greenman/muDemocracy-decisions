...
describe("DrizzleDecisionFieldRepository", () => {
  ...
  beforeEach(async () => {
    ...
    await db
      ...
      .where(
        sql`name LIKE 'Test%' OR name LIKE 'Minimal%' OR name LIKE 'Find%' OR name LIKE 'B Field%' OR name LIKE 'A Field%' OR name LIKE 'Field%' OR name LIKE 'Original%' OR name LIKE 'Updated%' OR name LIKE 'To Delete%' OR name LIKE 'Batch%' OR name LIKE 'Risk%' OR name LIKE 'Cost%' OR name LIKE 'Timeline%' OR name LIKE 'Text%' OR name LIKE 'Number%'`,
      );
  });
  ...
  describe("search", () => {
    it("should search across name, description, and category", async () => {
      ...
      // Search by category
      const catResults = inNamespace(await repository.search("context")).filter(
        (field) => field.name === "Timeline",
      );
  ...
  describe("findByType", () => {
    ...
    it("should return empty array for non-existent type", async () => {
      ...
      expect(
        inNamespace(results).filter(
          (field) => field.name.startsWith("Text Field") || field.name === "Number Field",
        ),
      ).toEqual([]);
    });
  });
