...
describe('web transcription server', () => {
  it('creates a session, accepts chunk uploads, reports status, and flushes on stop', async () => {
    ...
    const chunkResponse = await fetch(`${baseUrl}/sessions/${createPayload.sessionId}/chunks?filename=first.webm`, {
      method: 'POST',
      headers: {
    ...
    expect(provider.transcribe).toHaveBeenCalledWith(
      ...
      { filename: 'first.webm', language: 'en' },
    );
    expect(apiClient.postStreamEvent).toHaveBeenNthCalledWith(
      1,
      'meeting-browser-1',
      { text: 'hello world', startTimeSeconds: 0.2, sequenceNumber: 1 },
    );
    expect(apiClient.postStreamEvent).toHaveBeenNthCalledWith(
      2,
      'meeting-browser-1',
      { text: 'second line', startTimeSeconds: 1.1, sequenceNumber: 2 },
    );

    const statusResponse = await fetch(`${baseUrl}/sessions/${createPayload.sessionId}/status`);
    expect(statusResponse.status).toBe(200);
    const statusPayload = await statusResponse.json() as { status: string; bufferedEvents: number };
    ...
    const stoppedStatusResponse = await fetch(`${baseUrl}/sessions/${createPayload.sessionId}/status`);
